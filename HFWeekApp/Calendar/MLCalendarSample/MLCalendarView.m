...
@property (weak) IBOutlet NSButton *calendarTitle;
...
- (void) setDate:(NSDate *)date {
	...
	self.calendarTitle.title = budgetDateSummary;
}
...
- (void)setStartDate:(NSDate *)startDate endDate:(NSDate *)endDate {
    _startDate = [self toUTC:startDate];
    _endDate = [self toUTC:endDate];
    [self setDate:startDate];
}

- (void)setWeek:(NSString *)week {
    NSMutableArray *comps = [[self.calendarTitle.title componentsSeparatedByString: @", "] mutableCopy];
    [comps removeLastObject];
    [comps addObject:week];
    self.calendarTitle.title = [comps componentsJoinedByString: @", "];
}
...
- (IBAction)nextWeek:(id)sender {
    NSString *week = [[self.calendarTitle.title componentsSeparatedByString: @", "] lastObject];
    [self.delegate didSelectNextWeek: week];
}

- (IBAction)prevWeek:(id)sender {
    NSString *week = [[self.calendarTitle.title componentsSeparatedByString: @", "] lastObject];
    [self.delegate didSelectPreviousWeek:week];
}

- (IBAction)currentWeek:(id)sender {
    [self.delegate didSelectCurrentWeek];
}
